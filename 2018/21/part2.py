...
def execute_program(instructions, register):
	...
	while 0 <= ip < len_instructions:
		...
		# times that number is greater than register 2
		...
		# If we reach IP=28, track what numbers we have seen. As soon as it
		# loops, we can output the previous number and break out
		if ip == 28:
			if register[3] in seen_r0:
				print("Last value before loop for r0=%d" % (last_r0))
				break
			seen_r0.add(register[3])
			last_r0 = register[3]
