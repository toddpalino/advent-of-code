...
class Fighter:
	...
	# WARNING: This move method has a bug in the way it searches, but it doesn't show up until part 2. It's fixed there
	def move(self):
		...
		# If we get here, we didn't find a target, so we can't move
...
for y in range(len(map)):
	for x, c in enumerate(map[y]):
		...
		if c == 'E':
			units.append(Elf(space))
		elif c == 'G':
			units.append(Gnome(space))

in_combat = True
rounds = 0
while in_combat:
	# Order the list of units by row and column (steps in reverse)
	units.sort(key=methodcaller('x'))
	units.sort(key=methodcaller('y'))

	for unit in units:
		if living_targets(units, unit.target_klass) == 0:
			in_combat = False
			break
...
total_hp = sum(max(0, unit.hp) for unit in units if unit.is_alive())
