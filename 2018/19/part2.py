...
def execute_program(instructions, register):
	...
	while 0 <= ip < len_instructions:
		...
		# There is a loop that starts if we reach IP=3 which will increment register 5 until it reaches register 1
		# Shortcut it by setting the registers as they will be when it finishes
		if ip == 3:
			register[0] = sum(factors(register[1]))
			register[2] = register[1] + 1
			register[3] = 1
			register[4] = 16
			register[5] = register[1] + 1
			ip = 16
			continue
