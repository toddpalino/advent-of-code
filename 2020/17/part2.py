...
with open("input.txt") as f:
	actives = set((x, y, 0, 0) for y, line in enumerate(f.readlines()) for x, c in enumerate(line) if c == '#')
...
for _ in range(cycles):
	...
	for c in product(check_ranges[0], check_ranges[1], check_ranges[2], check_ranges[3]):
		# Generate a set of neighbors (don't include ourselves)
		neighbors = set(product(range(c[0]-1, c[0]+2), range(c[1]-1, c[1]+2), range(c[2]-1, c[2]+2), range(c[3]-1, c[3]+2)))
		neighbors.remove(c)
